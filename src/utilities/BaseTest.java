...
public abstract class BaseTest extends FrameworkUtility {
	...
	public void initlizeExtentReportsObject() {
		...
		// test.log(LogStatus.INFO, "Initializing extent reports..");
	...
	/************************************************************************************************************************/
	/*
	 * Starts the test. method.getName() returns the name of the current test
		...
		if (URL.trim().length() > 0) {
			...
			if (browserName.trim().equalsIgnoreCase("firefox")) {
				...
				service = new ChromeDriverService.Builder()
						.usingDriverExecutable(new File(FrameworkConstants.CHROME_DRIVER_PATH)).usingAnyFreePort()
						.build();
				service.start();
